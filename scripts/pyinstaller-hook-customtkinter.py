...
def _customtkinter_submodule_filter(module_name: str) -> bool:
    excluded_parts: tuple[str, ...] = (".tests", ".test", ".testing", ".examples", ".docs")
    return not (
        module_name.endswith(excluded_parts) or any(excluded_part in module_name for excluded_part in excluded_parts)
    )
