...
class TestCliCreateRun(CliTestCase):
    ...
    def test_stage_single_file_source_root_falls_back_to_copy_when_links_unavailable(self) -> None:
        """Single-file staging should copy when linking and symlinking are unavailable."""
        tmp_path: Path = self.make_temp_path()
        source_file: Path = tmp_path / "sample.bin"
        source_file.write_bytes(b"payload")
